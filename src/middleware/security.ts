...
export const createSecurityMiddleware = (config: SecurityConfig = {}): RequestHandler => {
...
export const createSessionValidator = (): RequestHandler => {
  const activeSessions = new Map<string, { lastActivity: number; ipAddress: string; userAgent: string }>();
  const SESSION_TIMEOUT = 30 * 60 * 1000; // 30 minutes
  const MAX_SESSIONS_PER_IP = 5;
  const sessionMutex = new Map<string, boolean>(); // Simple mutex to prevent race conditions
