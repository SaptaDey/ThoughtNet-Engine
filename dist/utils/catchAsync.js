...
const sanitizeError = (error) => {
    ...
    if (error) {
        ...
        if (error.message && typeof error.message === 'string') {
            const containsSensitive = sensitivePatterns.some(pattern => pattern.test(error.message));
...
const catchAsync = (fn) => (req, res, next) => {
    Promise.resolve(fn(req, res, next)).catch((error) => {
        ...
        res.status(sanitized.statusCode).json(Object.assign({ success: false, message: sanitized.message }, (process.env.NODE_ENV === 'development' &&
